...
export default function SignDocument({
  ...
  const { register = () => ({}), isReadOnly, setValue } = validationData || {};
  ...
  }, [validationUrl, documentObj, element.id, setValue]);
  ...
  useEffect(() => {
    ...
    const startPolling = async () => {
      ...
      if (!signed && isMounted) {
        timeoutId = setTimeout(startPolling, 2000);
      }
    };
  ...
  return (
    <div className="flex items-center gap-x-2">
      <input
        ...
        type={element.inputType || "hidden"}
        className={clsx("field-control", element.customClass, { "hidden": !element.inputType || element.inputType === "hidden" })}
        {...register(element.id)}
        disabled={isReadOnly}
      />

      <a 
        href={isSigned ? "#" : signatureLink} 
        target={isSigned ? undefined : "_blank"} 
